...
class Door(WorldObj):
    ...
    def toggle(self, env, pos, agent_id):
        ...
        if self.is_locked:
            if isinstance(env.agents[agent_id].carrying, Key) and env.agents[agent_id].carrying.color == self.color:
...
class Agent():
    """Controllable agent class. Has pos, dir, carrying.
    """
...
class MiniGridEnv(MultiAgentEnv, gym.Env):
    """
    ...
    @property
    def steps_remaining(self, agent_id=DEFAULT_AGENT_ID):
        return self.max_steps - self.agents[agent_id].step_count
    ...
    def __str__(self):
        ...
        for j in range(self.grid.height):
            ...
            for i in range(self.grid.width):
                # iterate over agents
                for agent in self.agents.values():
                    if i == agent.pos[0] and j == agent.pos[1]:
                        str += 2 * AGENT_DIR_TO_STR[agent.dir]
                        continue
    ...
    def _reward(self, agent_id=DEFAULT_AGENT_ID):
        """
        Compute the reward to be given upon success
        """
        return 1 - 0.9 * (self.agents[agent_id].step_count / self.max_steps)
    ...
    def place_agent(
        ...
        self.agents[agent_id].pos = None
        pos = self.place_obj(None, top, size, max_tries=max_tries)
        self.agents[agent_id].pos = pos

        if rand_dir:
            self.agents[agent_id].dir = self._rand_int(0, 4)

        return pos

    def dir_vec(self, agent_id=DEFAULT_AGENT_ID):
        """
        Get the direction vector for the agent, pointing in the direction
        of forward movement.
        """

        assert self.agents[agent_id].dir >= 0 and self.agents[agent_id].dir < 4
        return DIR_TO_VEC[self.agents[agent_id].dir]

    def right_vec(self, agent_id=DEFAULT_AGENT_ID):
        """
        Get the vector pointing to the right of the agent.
        """

        dx, dy = self.dir_vec(agent_id=agent_id)
        return np.array((-dy, dx))

    def front_pos(self, agent_id=DEFAULT_AGENT_ID):
        """
        Get the position of the cell that is right in front of the agent
        """

        return self.agents[agent_id].pos + self.dir_vec(agent_id)

    def get_view_coords(self, i, j, agent_id=DEFAULT_AGENT_ID):
        """
    ...
    def get_view_exts(self, agent_id=DEFAULT_AGENT_ID):
        """
        ...
        if self.agents[agent_id].dir == 0:
            topX = self.agents[agent_id].pos[0]
            topY = self.agents[agent_id].pos[1] - self.agent_view_size // 2
        # Facing down
        elif self.agents[agent_id].dir == 1:
            topX = self.agents[agent_id].pos[0] - self.agent_view_size // 2
            topY = self.agents[agent_id].pos[1]
        # Facing left
        elif self.agents[agent_id].dir == 2:
            topX = self.agents[agent_id].pos[0] - self.agent_view_size + 1
            topY = self.agents[agent_id].pos[1] - self.agent_view_size // 2
        # Facing up
        elif self.agents[agent_id].dir == 3:
            topX = self.agents[agent_id].pos[0] - self.agent_view_size // 2
            topY = self.agents[agent_id].pos[1] - self.agent_view_size + 1
        else:
    ...
    def relative_coords(self, x, y, agent_id=DEFAULT_AGENT_ID):
        """
    ...
    def gen_obs_grid(self, agent_id=DEFAULT_AGENT_ID):
        ...
        topX, topY, botX, botY = self.get_view_exts(agent_id=agent_id)

        grid = self.grid.slice(topX, topY, self.agent_view_size, self.agent_view_size)

        for i in range(self.agents[agent_id].dir + 1):
        ...
        if self.agents[agent_id].carrying:
            grid.set(*agent_pos, self.agents[agent_id].carrying)
        else:
            grid.set(*agent_pos, None)

        return grid, vis_mask

    def gen_obs(self, agent_id=DEFAULT_AGENT_ID):
        """
        ...
        obs = {
            'image': image,
            'direction': self.agents[agent_id].dir,
